function  Key = KOS_normalized_method_crowd_model(Model, varargin)
% The normalized version of the method in Karger et.al. 2011, where the messages are normalized by the degrees of the nodes, 
% that is, y_{ij} = sum_{k\in neib_i, k\neq j} x_{ki}/(degree_i-1) (as suggested by Chien-Ju Ho (http://www.cs.ucla.edu/~cjho/) through personal communication)
% This is equivalent to the original KOS method on regular bipartite graphs. 
% varargin: 
%       maxIter: maximum number of iteration.
%       TOL: error tolerance.       
%       verbose: control output information. 
%       initialMsg: intialization method.
%
%
...
for iter = 1:maxIter
    ...
    for i = 1:Ntask
        neib = NeibTask{i};
        for j = neib(:)'
            msg_I2J(i,j) = sum( full(L(i, neib(neib~=j))') .* full(msg_J2I(neib(neib~=j),i)) )  / (length(neib)-1);
        end
    end
    ...
    for j = 1:Nwork
        ...
        for i = neib(:)'
            ...
            msg_J2I(j,i) = sum( full(L(neib(neib~=i),j)) .* full(msg_I2J(neib(neib~=i), j)) ) /(length(neib)-1) + kappa;            
        end
    end
...
if ~isempty(zdx)
    fprintf('KOS_method_crowd_model: ties happens\n');
