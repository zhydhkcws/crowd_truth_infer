function negloglike =  Cal_Likelihood_MinimaxEntropy_Categorical_crowd_model(Model, Key, varargin)
%function negloglike =  Cal_Likelihood_MinimaxEntropy_Categorical_crowd_model(Model, Key, varargin)
% Calculating the negative log-likelihood for MinimaxEntropy_Categorical_crowd_model.m
%
% Qiang Liu @Jan 2013
%%
...
%if isfield(Key, 'soft_labels'), 
%    mu = Key.soft_labels;
%    negloglikeBound = - sum(sum(mu .* logp_task))  + sum(sum(log(mu+eps).*mu));
%end

negloglike = - sum(log(sum(exp(logp_task)/Ndom,1)));
%negloglike_joint = - sum(log(  sum(mu.*exp(logp_task), 1)));         
