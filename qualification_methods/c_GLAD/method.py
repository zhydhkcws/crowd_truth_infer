...
def inital_qualification(datafile):
    f = open(datafile, 'r')
    reader = csv.reader(f)
    next(reader)
    inital_weight = dict()
    ...
    for line in reader:
        ...
        if inital_weight.has_key(worker):
            inital_weight[worker] = inital_weight[worker] + temp
        else:
            inital_weight[worker] = temp

    for worker in inital_weight.keys():
          inital_weight[worker] = sigmoid(inital_weight[worker])
...
if __name__ == "__main__":
    ...
    inital_weight = inital_qualification(qualificationfile)

    e2lpd, weight = GLAD(e2wl,w2el,label_set).Run(1e-4,inital_weight)
